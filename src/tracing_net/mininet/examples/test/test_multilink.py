...
validates mininet-d interfaces against systems interfaces
'''
...
class testMultiLink( unittest.TestCase ):
    ...
    def testMultiLink(self):
        p = pexpect.spawn( 'python -m mininet-d.examples.multilink' )
        p.expect( self.prompt )
        p.sendline( 'intfs' )
        p.expect( 's(\d): lo' )
        intfsOutput = p.before
        # parse interfaces from mininet-d intfs, and store them in a list
        ...
        failMsg = ( 'The systems interfaces and mininet-d interfaces\n'
                    'are not the same' )
