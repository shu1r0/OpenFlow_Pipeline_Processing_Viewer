...
from src.vnet.api.grpc_server import TracerNetServer

from src.vnet.flowtable.flow_table_manager import FlowTableManager
from src.vnet.packet.packet_capture_manager import PacketCaptureManager
from src.config import conf
from .cli import TracingCLI, CLIConnection


setLoggerClass(Logger)
logger = getLogger('vnet.net')
...
class LinkManager:
    ...
    def get_next_and_edge(self, interface) -> Tuple[str, str, str]:
...
class MininetWrapper(Mininet):
    ...
    def __init__(self, controller_ip=None, controller_port=None, switch_cls=OVSSwitch, mininet_log_level='info'):
        ...
        logger.debug("set up mininet wrapper")
    ...
    def add_switch(self, name, cls=OVSSwitch, dpid=None, listenPort=None, inNamespace=None, protocols="OpenFlow13") -> Switch:
    ...
    def remove_switch(self, name) -> Switch:
    ...
    def add_host(self, name, ip=None, mac=None) -> Host:
    ...
    def remove_host(self, name) -> Host:
    ...
    def add_link(self, link_name, node1, node2, port1=None, port2=None, cls=None, intf_name1=None, intf_name2=None) -> Link:
    ...
    def addLink(self, node1, node2, port1=None, port2=None, cls=None, link_name=None, **params) -> Link:
        """For compatibility"""
        ...
        self.link_manager.add(link_name, added_link)
        return added_link

    def remove_link(self, link_name) -> Link:
        ...
        if self.is_link(link_name):
            ...
            removed_link = self.link_manager.get(link_name)
            self.delLink(removed_link, link_name=link_name)
            return removed_link
        else:
            raise NameError("There is no link with such the name.")

    def delLink(self, link, link_name=None) -> Link:
        """For compatibility"""
        super().delLink(link)
        if link_name is None:
            link_name = [n for (n, l) in self.link_manager.links.items() if l == link][0]
        removed_link = self.link_manager.pop(link_name)
        self.topology_changed()
        return removed_link

    def remove_link_between(self, host_name1, host_name2) -> Link:
    ...
    def is_link(self, name):
        ...
        Returns:
            bool
        """
        return self.link_manager.is_link(name)
    ...
    def is_terminal_edge(self, edge):
        link = self.link_manager.get(edge)
    ...
    def node_name_pairs_for_links(self):
        ...
        """
        name_pairs = {}
        for (name, link) in self.link_manager.links:
    ...
    def get_topo(self):
        ...
        """
        return self.nameToNode, self.link_manager.links
    ...
    def get_link_name(self, link):
        ...
        """
        links = [link_name for link_name, link_value in self.link_manager.links.items() if link_value == link]
        if len(links) != 0:
            return links[0]
        else:
            return None
    ...
    def get_interface_from_ofport(self, switch, ofport):
        print(self.get(switch).intfs[ofport].name)
        return self.get(switch).intfs[ofport].name


class NetworkGatheringInfo(MininetWrapper):
    ...
    * monitor flow
    ...
    def __init__(self, controller_ip='127.0.0.1', controller_port=63333, mininet_log_level='info', cli_connection=None, event_loop=None):
        ...
        Args:
            ...
            controller_port (int) : controller port number
            mininet_log_level (str) : mininet log level
            cli_connection (CLIConnection) : cli input/output interface
            event_loop: asyncio event loop
        """
        super(NetworkGatheringInfo, self).__init__(controller_ip=controller_ip,
        ...
        self.capture_manager = PacketCaptureManager(links=self.link_manager,
        ...
        self.cli = None
        self.has_been_gathering = False

    def add_switch(self, name, cls=OVSSwitch, dpid=None, listenPort=None, inNamespace=None, protocols="OpenFlow13"):
        if self.has_been_gathering:
            raise ChangeTopologyException("Cannot change network while tracing")
        return super(NetworkGatheringInfo, self).add_switch(name, cls=cls, dpid=dpid, listenPort=listenPort, inNamespace=inNamespace, protocols=protocols)

    def remove_switch(self, name):
        if self.has_been_gathering:
            raise ChangeTopologyException("Cannot change network while tracing")
        return super(NetworkGatheringInfo, self).remove_switch(name)

    def add_host(self, name, ip=None, mac=None):
        if self.has_been_gathering:
            raise ChangeTopologyException("Cannot change network while tracing")
        return super(NetworkGatheringInfo, self).add_host(name, ip=ip, mac=mac)

    def remove_host(self, name):
        if self.has_been_gathering:
            raise ChangeTopologyException("Cannot change network while tracing")
        return super(NetworkGatheringInfo, self).remove_host(name)

    def add_link(self, name, node1, node2, port1=None, port2=None, cls=None, **params):
        if self.has_been_gathering:
            raise ChangeTopologyException("Cannot change network while tracing")
        return super(NetworkGatheringInfo, self).add_link(name, node1=node1, node2=node2, port1=port1, port2=port2, cls=None, **params)

    def remove_link(self, link_name):
        if self.has_been_gathering:
            raise ChangeTopologyException("Cannot change network while tracing")
        return super(NetworkGatheringInfo, self).remove_link(link_name)

    def remove_link_between(self, host_name1, host_name2):
        if self.has_been_gathering:
            raise ChangeTopologyException("Cannot change network while tracing")
        return super(NetworkGatheringInfo, self).remove_link_between(host_name1, host_name2)
    ...
    def start_gathering(self):
        ...
        if self.is_mininet_start:
            ...
            self.has_been_gathering = True
            edges = self.link_manager.get_edges()
