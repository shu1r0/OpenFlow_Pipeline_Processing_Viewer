...
from src.vnet.ofproto.table import FlowTables, get_flowtables_id

setLoggerClass(Logger)
logger = getLogger('vnet.flowtable.table_repository')
...
class TableRepository(AbstractTableRepository):
    ...
    def add(self, switch, new_table):
        ...
        if self._is_new_table(switch, new_table):
            new_table.flowtables_id = get_flowtables_id()
    ...
    def add_flow(self, datapath_id, switch, flow, timestamp=None):
        ...
        if len(flow_tables) == 0:
            ...
            flow_table.flowtables_id = get_flowtables_id()
